<?php
	...
	$empat1=array();
	$total1=array();
	echo "<a href='print_dosen.php?periode=".$periode."&kode_jurusan=".$jurusan."' class='btn btn-primary pull-right'>Print</a> ";
	include_once '../lib/Database.php';
	$db = new Database();
	$res= $db->select("SELECT * FROM mv_rekap_quesioner_dosen where periode_kode = '$periode' and jur = '$jurusan' AND nama_dosen<>'' ORDER BY nama_dosen");
	...
	$count=0;
		for($i=0;$i<count($nama_mk);$i++){
			...
			echo "<tr>";
			echo "<td width=300>".$nama_mk[$i]."</td>";
			...
			if($nama_dosen[$i+1]!=$nama_dosen[$i]){
				...
				echo "<td width=200 class='text-left'>Total MHS:</td>";
